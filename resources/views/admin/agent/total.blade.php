...
th { background: rgb(243, 243, 243); border-color: #BBB; }
td { border-color: rgb(247, 246, 246); }
...
<div id="invoice">
    <header >
      <h1>Abrechnung</h1>
      <span><img style="float: right;" src="{{ asset('public/hh.png') }}" width="160"></span>
    </header>
    <article>
      ...
      <table class="inventory">
        <thead>
          <tr>
            ...
            <th><span >Art</span></th>
            <th><span >Gesellschaft</span></th>
            <th><span >Provision</span></th>
          </tr>
        </thead>
        <tbody>
        @foreach ($apps as $in)
      ...
      <table class="balance">
        <tr>
          <th><span >Total Provisionen</span></th>
      ...
      <table class="sign">
        <tr>
            <th><span>Stornokonto Alt</span></th>
            <td><span data-prefix>CHF</span></td>
        </tr>
        <tr>
...
<script>
    ...
    function bvg(){
        var TotalAfterDone = parseFloat($('#TotalAfterDone').text());
        var BVG = ((parseFloat($('#BVG').text()))*100)/TotalAfterDone;
        $('#TotalBVG').text(BVG.toFixed(2));
    }

    function TotalTaxPer(){
        var NBU = parseFloat($('#NBU').text());
        var KTG = parseFloat($('#KTG').text());
        var ALV = parseFloat($('#ALV').text());
        var AHV = parseFloat($('#AHV').text());
        var BVG = parseFloat($('#BVG').text());

        var TotalTaxes = NBU+KTG+ALV+AHV+BVG;
